...
function describeKey(key: string, description: string) {
  return `${pc.dim(`${ARROW}Press`)} ${key} ${pc.dim(description)}`;
}

const TestProgressLabel = pc.bold('Interactive Test Progress');
...
export default class FailedTestsInteractiveMode {
  ...
  private _drawUIDoneWithSkipped() {
    ...
    if (this._skippedNum > 0) {
      const skippedText = pc.bold(
        pc.yellow(`${pluralize('test', this._skippedNum)} skipped`),
      );
