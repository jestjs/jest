...
describe('.toBeWithin()', () => {
  [
    ...
    it(`{pass: true} expect(${within}).toBeWithin(${small}, ${big})`, () => {
      jestExpect(small).toBeWithin(small, big);
    });

    it(`{pass: false} expect(${big}).toBeWithin(${small}, ${within})`, () => {
      jestExpect(big).not.toBeWithin(small, within);
    });

    it(`throws: [${small}, ${within}, ${big}]`, () => {
      expect(() =>
        jestExpect(within).not.toBeWithin(small, big),
      ).toThrowErrorMatchingSnapshot();

      expect(() =>
        jestExpect(big).toBeWithin(small, within),
      ).toThrowErrorMatchingSnapshot();
    });
  });
  ...
  [
    ...
    test(`equal numbers: [${n1}, ${n2}, ${n3}]`, () => {
      jestExpect(n2).toBeWithin(n1, n3);

      expect(() =>
        jestExpect(n2).not.toBeWithin(n1, n3),
      ).toThrowErrorMatchingSnapshot();
