...
const ensureNoDuplicateConfigs = (
  ...
  for (const config of parsedConfigs) {
    ...
    if (configPathMap.has(configPath)) {
      const message = `Whoops! Two projects resolved to the same config path: ${pc.bold(
        String(configPath),
      )}:

  Project 1: ${pc.bold(projects[parsedConfigs.indexOf(config)])}
  Project 2: ${pc.bold(
    projects[parsedConfigs.indexOf(configPathMap.get(configPath))],
  )}

This usually means that your ${pc.bold(
        '"projects"',
