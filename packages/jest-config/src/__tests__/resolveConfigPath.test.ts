...
test('reports a package.json reference to an automatically discovered config as a duplicate', async () => {
  writeFiles(DIR, {
    'jest.config.js': 'module.exports = {};',
    'package.json': JSON.stringify({jest: './jest.config.js'}),
  });

  await expect(resolveConfigPath(DIR, DIR)).rejects.toThrow(
    MULTIPLE_CONFIGS_ERROR_PATTERN,
  );
  await expect(resolveConfigPath(DIR, DIR, true)).resolves.toBe(
    path.resolve(DIR, 'jest.config.js'),
  );
  await expect(resolveConfigPath('jest.config.js', DIR)).resolves.toBe(
    path.resolve(DIR, 'jest.config.js'),
  );
});
