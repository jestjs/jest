...
**`enableSymlinks` symlink semantics.** When `false` (default), symlinks are excluded by all walkers. When `true`: `find` (fdir path) passes `enableSymlinks` to `walk()`, which sets fdir's `resolveSymlinks: true` — symlinks-to-files are included at their link path with the target's mtime/size; symlinks-to-directories are recursed into. `findNative` (shell `find` path) does NOT support `enableSymlinks` — it always uses `-type f` only. Do NOT expose fdir's raw `excludeSymlinks`/`resolveSymlinks` through `WalkOptions` — fdir's "mode 2" (neither flag set) hands callers raw lstat'd symlink entries with symlink stats rather than target stats, which no haste-map call site wants.
