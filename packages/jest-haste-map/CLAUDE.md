...
- `watchers/ChangeQueue.ts` — 30 ms periodic flush (a `setInterval` that emits whatever has queued; it does not reset on activity the way a debounce would), copy-on-write for the live map, file-processing dispatch. Two separate guards drop repeats: a `Set<string>` of `type:path:mtime` keys discards duplicate events within a frame, and a direct comparison against the tracked `mtime` discards `change` events for files that were only accessed.
- `ModuleMap.ts` — `toJSON`/`fromJSON` are not just a cache concern: `jest-runner` serialises the module map into every test worker and rebuilds it there, so this pair is on the hot path of any parallel run and a serialization bug surfaces as a worker-side failure.
...
- Overlapping `roots` (e.g. `<rootDir>` together with `<rootDir>/src`) reach the crawlers as given. Watchman collapses them — `watch-project` maps both to one watch root — but `find()` walks each root separately, so the shared subtree is traversed twice. The resulting map is still correct (entries are keyed by path relative to `rootDir`). `WalkOptions.statCache` is shared across the per-root walks, which spares some of the repeated `lstat` calls, but only best-effort: the walks run concurrently, so two of them can both miss a path before either has cached it. The traversal itself is always duplicated.
