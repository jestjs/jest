...
- Overlapping `roots` (e.g. `<rootDir>` together with `<rootDir>/src`) reach the crawlers as given. Watchman collapses them — `watch-project` maps both to one watch root — but `find()` walks each root separately, so the shared subtree is traversed twice. The resulting map is still correct (entries are keyed by path relative to `rootDir`) and `WalkOptions.statCache` is shared across the per-root walks so the `lstat` calls are not repeated; the duplicated work is the directory traversal itself.
