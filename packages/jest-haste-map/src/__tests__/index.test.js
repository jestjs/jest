...
describe('HasteMap', () => {
  ...
  it('warns on duplicate mock files', () => {
    ...
    mockFs[
      path.join('/project', 'fruits1', '__mocks__', 'subdir', 'Blueberry.js')
    ] = `
      // Blueberry
    `;
    mockFs[
      path.join('/project', 'fruits2', '__mocks__', 'subdir', 'Blueberry.js')
    ] = `
      // Blueberry too!
    `;
  ...
  describe('file system changes processing', () => {
    ...
    describe('recovery from duplicate module IDs', () => {
      async function setupDuplicates(hm) {
        ...
        expect(
          hasteFS.exists(path.join('/project', 'fruits', 'another', 'Pear.js')),
        ).toBe(true);
        try {
          ...
          expect(error.duplicatesSet).toEqual(
            createMap({
              ...
              [path.join('/project', 'fruits', 'another', 'Pear.js')]: H.MODULE,
            }),
          );
      ...
      hm_it('recovers when the most recent duplicate is fixed', async hm => {
        await setupDuplicates(hm);
        mockFs[path.join('/project', 'fruits', 'another', 'Pear.js')] = null;
