...
- `[jest-runtime]` Reduce per-require overhead: skip module ID resolution when no mock can apply, answer core modules before probing for a manual mock, share one `require.cache` proxy across modules, cache empty files, and stop rebuilding the injected `jest` object and parameter list per module ([#16376](https://github.com/jestjs/jest/pull/16376))
