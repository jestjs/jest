...
- `[expect, jest-circus, jest-jasmine2, jest-snapshot, @jest/snapshot-utils]` [**BREAKING**] Give each test its own snapshot keys, so tests sharing a full name no longer take each other's snapshots under `--randomize` or `test.concurrent`. The second and later tests sharing a name now write `name 2.1` instead of continuing the first test's numbering. Re-record those snapshots with `--updateSnapshot` and commit them before upgrading — under `--ci` the new keys are missing and the old ones obsolete, and `--ci` writes no snapshots ([#16349](https://github.com/jestjs/jest/pull/16349))
- `[jest-snapshot, @jest/snapshot-utils]` [**BREAKING**] Join a snapshot hint onto the test name as `name › hint` instead of `name: hint`, so a colon in a test title is no longer read as the start of a hint. A snapshot the test never reached is now attributed to the test that took it, and one left by a deleted test is reported obsolete instead of being claimed by a namesake. Re-record hinted snapshots with `--updateSnapshot` ([#16349](https://github.com/jestjs/jest/pull/16349))
...
- `[jest-circus, jest-jasmine2, jest-snapshot, jest-types]` [**BREAKING**] Report a snapshot obsolete when the test that took it is gone, even where a skipped or failed test shares its full name — `AssertionResult` carries the new `nameOccurrence` so the two are told apart ([#16349](https://github.com/jestjs/jest/pull/16349))
