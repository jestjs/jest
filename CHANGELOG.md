...
- `[jest-docblock]` Add `extractAll()`, which returns every docblock that precedes the first statement of a file instead of only the first one ([#16425](https://github.com/jestjs/jest/pull/16425))
...
- `[jest-runner]` Honor the `@jest-environment` and `@jest-environment-options` pragmas when they are not in the first comment of a test file, so a pragma placed after a license header or a line comment is no longer silently ignored ([#16425](https://github.com/jestjs/jest/pull/16425))
