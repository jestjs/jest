...
- `[jest-runtime]` Key builtin modules in the ESM registry by one canonical specifier, so `import * as a from 'fs'` and `import * as b from 'node:fs'` share a namespace object as they do in Node ([#16340](https://github.com/jestjs/jest/pull/16340))
- `[jest-runtime]` Answer `import.meta.resolve()` for a builtin with its `node:` specifier, instead of a `file://` URL pointing at a path that does not exist ([#16340](https://github.com/jestjs/jest/pull/16340))
