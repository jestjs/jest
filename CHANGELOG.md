...
- `[jest-each]` Escape a table row's keys before building the `$variable` interpolation `RegExp`, so a column name containing a regex metacharacter (for example `count(*)`) no longer fails the whole table with `Invalid regular expression`, and a row with no keys no longer replaces a bare `$` in the title ([#16345](https://github.com/jestjs/jest/pull/16345))
