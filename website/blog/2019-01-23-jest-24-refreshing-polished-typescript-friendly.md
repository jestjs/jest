...
- To reduce the amount of magic Jest performs to “just work™”, in this release we decided to drop automatic injection of `regenerator-runtime` for transpiled async code to work. It's not always necessary (e.g. in Node 6+) and we believe it's the user's responsibility to handle it, which is already done if you use `@babel/preset-env` and `@babel/runtime` with correctly set up `targets` configuration. Please see our [Using Babel docs](/docs/en/getting-started#using-babel) for more information.
