...
_Note: when adding an additional code preprocessors, this will overwrite the default config and `babel-jest` is no longer automatically loaded. If you want to use it to compile JavaScript or Typescript, it has to be explicitly defined by adding `{".(js|jsx)$": "babel-jest"}` or `{".(ts|tsx)$": "babel-jest"}` respectively to the transform array. See [babel-jest plugin](https://github.com/facebook/jest/tree/master/packages/babel-jest#setup)_
