...
You can use `expect.extend` to add your own matchers to Jest (but first, have a look at [jest-extended](https://github.com/jest-community/jest-extended) for many additional matchers). For example, let's say that you're testing a number utility library and you're frequently asserting that numbers appear within particular ranges of other numbers. You could abstract that into a `toBeWithinRange` matcher:
