...
> Note: The test will pass even if you remove `class` from the `<Link>` used in any other file.
> Example: Using `<Link />` will pass the test as the test doesn't know the usage of `Link` component in other files. 
> You always need to investigate your component file rather than it's usage in other files.
